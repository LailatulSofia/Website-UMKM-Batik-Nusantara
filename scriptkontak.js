document.addEventListener("DOMContentLoaded", () => {
    ...
    function validateKontak() {
        ...
        else if (hpVal.length < 10 || hpVal.length > 13)
        errors.push("Nomor HP minimal 10 digit dan maksimal 13 digit.");
        ...
        if (selected < today)
            errors.push("Tanggal tidak boleh sebelum hari ini.");
        }
    ...
    function validateWebinarField() {
        ...
        if (!namaVal) {
        setError("errorNamaWebinar", "Nama lengkap wajib diisi.");
        valid = false;
        } else if (namaVal.length < 3) {
        setError("errorNamaWebinar", "Minimal 3 karakter.");
        valid = false;
        ...
        const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailVal) {
        setError("errorEmailWebinar", "Email wajib diisi.");
        valid = false;
        } else if (!emailPattern.test(emailVal)) {
        setError("errorEmailWebinar", "Format email tidak valid.");
        valid = false;
        } else if (emailVal.endsWith("@yahoo.com")) {
        setError(
            "errorEmailWebinar",
            "Email tidak boleh menggunakan domain yahoo.com."
        );
        valid = false;
        ...
        const hpPattern = /^[0-9]+$/;
        if (!hpVal) {
        setError("errorHpWebinar", "No HP wajib diisi.");
        valid = false;
        } else if (!hpPattern.test(hpVal)) {
        setError("errorHpWebinar", "No HP harus angka saja.");
        valid = false;
        } else setError("errorHpWebinar", "");

        // Topik: wajib pilih
        if (!topik.value) {
        setError("errorTopikWebinar", "Wajib pilih topik webinar.");
        valid = false;
        } else setError("errorTopikWebinar", "");

        // Checkbox: wajib centang
        if (!setuju.checked) {
        setError("errorSetujuWebinar", "Wajib menyetujui syarat & ketentuan.");
        valid = false;
    ...
    if (formWebinar) {
        ...
        successWebinar.textContent =
            "Pendaftaran webinar berhasil! Sampai jumpa di acara 😊";
        formWebinar.reset();
